...
set "command=%python%%imgtool% flash --layout %preprocess_bl2_file% -b wrpb_start -m  RE_LOADER_WRP_START -s 1 -p 0 %hardening% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%imgtool% flash --layout %preprocess_bl2_file% -b wrpb_end -m  RE_LOADER_WRP_END -s 1 -p 0 %hardening% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
