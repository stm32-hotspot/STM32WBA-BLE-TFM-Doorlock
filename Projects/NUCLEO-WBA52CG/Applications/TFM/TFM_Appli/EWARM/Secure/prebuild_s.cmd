...
copy %CONFIG%\tfm_app_rot.yaml %projectdir% /Y
copy %CONFIG%\tfm_platform.yaml %projectdir% /Y
copy %CONFIG%\tfm_radio_service.yaml %projectdir% /Y
copy %CONFIG%\tfm_doorlock_service.yaml %projectdir% /Y
dir %TOOLS% > %1\output.txt 2>&1

echo "## tfm_parse_manifest_list ##"
#echo %projectdir%
#echo "%TOOLS%\dist\tfm_parse_manifest_list\tfm_parse_manifest_list.exe -o %projectdir%\g -m %projectdir%\tfm_manifest_list.yaml -f %projectdir%\tfm_generated_file_list.yaml"
echo "%python% %TOOLS%\dist\pyscript\tfm_parse_manifest_list.py -o %projectdir%\g -m %projectdir%\tfm_manifest_list.yaml -f %projectdir%\tfm_generated_file_list.yaml" >> output_cmd.txt
#%python% %TOOLS%\dist\pyscript\tfm_parse_manifest_list.py -o %projectdir%\g -m %projectdir%\tfm_manifest_list.yaml -f %projectdir%\tfm_generated_file_list.yaml >> %1\output_parse.txt 2>&1
%TOOLS%\dist\tfm_parse_manifest_list\tfm_parse_manifest_list.exe -o %projectdir%\g -m %projectdir%\tfm_manifest_list.yaml -f %projectdir%\tfm_generated_file_list.yaml >> %1\output_parse.txt 2>&1
