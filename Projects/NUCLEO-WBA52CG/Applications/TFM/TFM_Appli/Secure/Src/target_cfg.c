...
#ifdef ENABLE_BLE_STACK
    /* Configure the peripherals space to access OTP */
    {
        TFM_NS_REGION_OTP,
        FLASH_OTP_BASE,
...
static void gtzc_internal_flash_priv(uint32_t offset_start, uint32_t offset_end)
{
  ...
  if (offset_start & (FLASH_AREA_IMAGE_SECTOR_SIZE - 1))
    Error_Handler();
  /*  1f is for 32 bits */
  for (index = block_start & ~0x1f; index < (4 << 5) ; index++) {
    /* clean register on index aligned */
    if (!(index & 0x1f)) {
      regwrite = 0x0;
    }
...
#endif
{
  ...
  if (uFlowStage == FLOW_STAGE_CFG)
  {
    ...
    //GTZC_MPCBB6_S->CFGLOCK=MPCBB_LOCK(SRAM6_BASE);
    
    HAL_GTZC_TZSC_ConfigPeriphAttributes(GTZC_PERIPH_USART1, GTZC_TZSC_PERIPH_NSEC | GTZC_TZSC_PERIPH_NPRIV);
    HAL_GTZC_TZSC_ConfigPeriphAttributes(GTZC_PERIPH_ADC4, GTZC_TZSC_PERIPH_NSEC | GTZC_TZSC_PERIPH_NPRIV);
    HAL_GTZC_TZSC_ConfigPeriphAttributes(GTZC_PERIPH_RADIO, GTZC_TZSC_PERIPH_NSEC | GTZC_TZSC_PERIPH_NPRIV);
    HAL_GTZC_TZSC_ConfigPeriphAttributes(GTZC_PERIPH_RAMCFG, GTZC_TZSC_PERIPH_NSEC | GTZC_TZSC_PERIPH_NPRIV);
#else
...
#else
    if (HAL_DMA_ConfigChannelAttributes(&DMAHandle, DMA_CHANNEL_NSEC | DMA_CHANNEL_NPRIV) != HAL_OK)
    {
        Error_Handler();
    }
#endif /*ENABLE_BLE_STACK*/
